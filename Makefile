...
$(BUILD_DIR)/lib$(b)/unityfix/unityfix.so: src/unityfix.c
	mkdir -p $(BUILD_DIR)/lib$(b)/unityfix
	/compat/linux/bin/cc -m$(b) $(CFLAGS) -fPIC -shared -o $(.TARGET) src/unityfix.c -pthread -ldl
